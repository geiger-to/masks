Masks::Seeder.run do
  seed_tenant('default') do |tenant|
    default = seed_profile(tenant, 'default') do |profile|
    ...
    tenant.client = if (client_key = tenant.setting(:client))
      seed_client(
        default, client_key,
        name: tenant.name,
    ...
    tenant.admin = if (admin_key = tenant.setting(:admin))
      seed_client(
        default,
        admin_key,
        ...
        redirect_uris: [tenant.redirect_uri, "#{tenant.redirect_uri}*"],
        scopes: ['openid', 'email', 'masks:manage'],
        consent: false,
        profile: default
      )
    end
