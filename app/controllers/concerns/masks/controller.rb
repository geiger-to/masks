module Masks
  ...
  module Controller
    ...
    def passed?
      current_actor && current_session.passed?
    end
    ...
    # @visibility public
    def current_session
      @current_session ||= request.env[Masks::Middleware::SESSION_KEY]
    end
    ...
    def build_access(name)
      @current_access = Masks.access(name, current_session)
    ...
    def masked?(type: nil)
      if type &&
           !Array.wrap(type).map(&:to_s).include?(current_session.mask.type)
        false
      else
        current_session.passed?
      end
    end
