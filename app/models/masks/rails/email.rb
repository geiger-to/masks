  ...
  module Rails
    class Email < ApplicationRecord
      ...
      def notify!(session)
        ...
        if valid?
          save
          ActorMailer.with(session: session, email: self).verify_email.deliver_now
        end
      end
      ...
      def expired?
        ...
        notified_at <= (Masks.configuration.lifetimes[:email_verification] || 1.hour).ago
      end

      def taken?
        self.class.where(email: email, verified: true).any?
      end
