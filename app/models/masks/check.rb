module Masks
  ...
  # - +falied?+ - true if attempts were made and any were denied
  #
  ...
  class Check < ApplicationModel
    ...
    # @return Boolean
    def passed?
      return true if optional? && !falied?
    ...
    def attempt_approved?(id)
      ...
      return self.approved if duration == "request"
      ...
      time ? time + ActiveSupport::Duration.parse(self.duration) > Time.current : false
    end
    ...
    def approve!(id, **opts)
      ...
      merge_attempt(id, opts.merge(approved_at: Time.current.iso8601, skipped_at: nil))
    end
    ...
    def skip!(id, **opts)
      ...
      merge_attempt(id, opts.merge(approved_at: nil, skipped_at: Time.current.iso8601))
    end
    ...
    def falied?
      ...
      attempts.any? { |id, opts| !attempt_approved?(id) && !attempt_skipped?(id) }
    end
