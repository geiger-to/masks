module Masks
  module Credentials
    class Email < Masks::Credential
      checks :actor

      def lookup
        return if self.actor || !email

        actor = config.find_actor(session, email: email)
        actor ||= config.build_actor(session, email: email, nickname: generate_nickname(email))
        actor
      end

      def maskup
        approve! if actor && email && valid?
      end

      private

      def email
        @email ||=
          [session_params[:email], session_params[:nickname]].find do |param|
            ValidateEmail.valid?(param)
          end
      end

      def generate_nickname(email)
        return email if !nickname_format || nickname_format.match?(email)

        parts = email.split("@")
        name = parts[0].gsub(/[^\w\d]/, "")

        prefix_nickname("#{name.downcase.slice(0, 16)}#{SecureRandom.hex.slice(0, 8)}")
      end
    end
  end
end
