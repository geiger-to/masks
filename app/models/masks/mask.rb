module Masks
  ...
  class Mask < ApplicationModel
    ...
    # @return [String]
    def actor
      super || type_config.fetch(:actor, nil) || config.models[:actor]
    end
    ...
    # @return [String]
    def actor_scope
      @actor_scope ||= (actor.constantize unless skip)
    end
    ...
    def credentials
      ...
      merged = [*(type_config&.fetch(:credentials) || []), *(super || [])].uniq
      merged.map do |cls|
        case cls
        when Class
          cls
    ...
    # Returns whether or not the mask matches the passed actor.
    #
    # The behaviour of this method depends on the mask's configuration.
    # For example, an anonymous actor will return true only if the
    # mask's +#allow_anonymous?+ method returns true.
    #
    # @return [Boolean]
    def matches_actor?(actor)
      ...
      return matches_scopes?(actor.scopes)
    end
