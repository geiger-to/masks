module Masks
  ...
  class Session < ApplicationModel
    ...
    class << self
      def mask!(*args, **opts)
        new(*args, **opts).tap { |session| session.mask! }
      end
    ...
    #
    # @param [Hash] a hash of extra data to merge
    def extras(**opts)
      @extras ||= {}
    ...
    #
    # @param [Symbol|String] the name of the key
    # @return [any]
    def extra(key)
    ...
    def passed_at
      ...
      checks.values.map(&:passed_at).compact.sort.last
    end
