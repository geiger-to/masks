    ...
    class Authorization
      ...
      def initialize(env, **opts)
        ...
        @app = Rack::OAuth2::Server::Authorize.new do |req, res|
          @client = session.config.model(:openid_client).find_by(key: req.client_id)
          ...
          if @client.response_types.include?(Array(req.response_type).collect(&:to_s).join(' '))
            if actor
      ...
      def approved!(req, res)
        ...
        if response_types.include? :code
          authorization = actor.openid_authorizations.create!(
        ...
        if response_types.include? :token
          access_token = actor.openid_access_tokens.create!(
            openid_client: client,
            scopes: @scopes
          )
        ...
        if response_types.include? :id_token
          id_token = actor.openid_id_tokens.create!(
            openid_client: @client,
            nonce: req.nonce
          )
