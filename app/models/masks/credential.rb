module Masks
  ...
  class Credential < ApplicationModel
    ...
    delegate :config, :actor, :session_params, :account_params, :params, :writable?, to: :session
    ...
    def mask!
      ...
      return if check && check.passed? && check.attempts[slug] && valid?
    ...
    def backup!
      self.passed_at = Time.current if check && check.passed? && check.attempt_approved?(slug)
    ...
    delegate :optional?, :passed?, :skipped?, :invalidated?, to: :check, allow_nil: true
