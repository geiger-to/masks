module Masks
  module Prompts
    class OIDC
      ...
      before_entry { prompt! oauth_request.error if oauth_request.error }

      # if session.flag? :oidc
      #   attempt = oidc_attempt

      #   self.prompt = attempt.error if attempt.error
      # end

      # session.bag :attempts, current: attempt_id if valid_oidc_request?

      # callback :around do |block|
      #   oidc =
