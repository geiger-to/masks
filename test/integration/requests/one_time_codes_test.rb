require "test_helper"

module Masks
  # [GET|POST] /masks/one-time-codes
  #
  # view and configure one-time-passwords, a form of secondary authentication.
  class OneTimeCodesTest < ActionDispatch::IntegrationTest
    include Masks::TestHelper

    test "GET requires a valid session" do
      get "/masks/one-time-codes"

      assert_equal 302, status
    end

    test "GET is only available to logged in participants" do
      signup_as "test", status: 302 do
        get "/masks/one-time-codes"
        assert_equal 200, status
      end
    end

    test "POST requires a valid session" do
      post "/masks/one-time-codes", params: { session: { password: "invalid" } }

      assert_equal 0, Masks::Rails::Actor.count
      assert_equal 302, status
    end

    test "POST requires a valid password" do
      signup_as "test" do
        post "/masks/one-time-codes", params: { session: { password: "invalid" } }
        assert_equal 302, status
      end

      actor = Masks::Rails::Actor.find_by(nickname: "@test")
      refute actor.factor2?
    end

    test "POST sets a totp_secret with a valid code" do
      signup_as "test" do
        totp_secret = ROTP::Base32.random
        totp_code = ROTP::TOTP.new(totp_secret).at(Time.current)

        post "/masks/one-time-codes",
             params: {
               one_time_code: {
                 code: "invalid",
                 secret: totp_secret
               },
               session: {
                 password: "password"
               }
             }
        assert_equal 302, status
      end

      actor = Masks::Rails::Actor.find_by(nickname: "@test")
      refute actor.factor2?
      refute actor.totp_secret
    end

    test "POST sets a totp_secret with a valid secret" do
      signup_as "test" do
        totp_secret = ROTP::Base32.random
        totp_code = ROTP::TOTP.new(totp_secret).at(Time.current)

        post "/masks/one-time-codes",
             params: {
               one_time_code: {
                 code: totp_code,
                 secret: "invalid"
               },
               session: {
                 password: "password"
               }
             }
        assert_equal 302, status
      end

      actor = Masks::Rails::Actor.find_by(nickname: "@test")
      refute actor.factor2?
      refute actor.totp_secret
    end

    test "POST sets a totp_secret with a valid password, secret, and code" do
      signup_as "test" do
        add_one_time_code
      end

      actor = Masks::Rails::Actor.find_by(nickname: "@test")
      assert actor.factor2?
      assert actor.totp_secret
    end

    test "DELETE removes a totp_secret with a valid password" do
      signup_as "test" do
        add_one_time_code

        actor = Masks::Rails::Actor.find_by(nickname: "@test")
        assert actor.factor2?
        assert actor.totp_secret

        delete "/masks/one-time-codes", params: { session: { password: "password" } }
        assert_equal 302, status

        refute actor.reload.factor2?
        refute actor.reload.totp_secret
      end
    end

    test "DELETE keeps a totp_secret with an invalid password" do
      signup_as "test" do
        add_one_time_code

        actor = Masks::Rails::Actor.find_by(nickname: "@test")
        assert actor.factor2?
        assert actor.totp_secret

        delete "/masks/one-time-codes", params: { session: { password: "invalid" } }
        assert_equal 302, status

        assert actor.reload.factor2?
        assert actor.reload.totp_secret
      end
    end

    test "POST /masks/session requires a one-time-code after enabling" do
      signup_as "test" do
        add_one_time_code
      end

      login_as "test" do
        refute_logged_in
      end

      actor = Masks::Rails::Actor.find_by!(nickname: "@test")
      actor.totp.at(Time.current)

      login_as "test", one_time_code: actor.totp.at(Time.current) do
        assert_logged_in
      end
    end

    test "POST /masks/session does not require re-entering a username/password" do
      signup_as "test" do
        add_one_time_code
      end

      login_as "test" do
        refute_logged_in

        actor = Masks::Rails::Actor.find_by!(nickname: "@test")
        code = actor.totp.now

        post "/masks/session", params: { session: { one_time_code: code } }

        assert_logged_in
      end
    end

    def dummy_app?
      true
    end
  end
end
