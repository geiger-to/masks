def cli_access(access_name, **opts, &block)
  access = nil
...
namespace :masks do
  ...
  task :signup, %i[nickname password] => :boot do |task, args|
    access = cli_access("actor.signup", args: args)
  ...
  task :assign_scopes, %i[actor scopes] => :boot do |task, args|
    ...
    access.assign_scopes(args[:scopes].split(";")) if access
  ...
  task :change_password, %i[actor password] => :boot do |task, args|
    ...
    access.change_password(args[:password]) if access
