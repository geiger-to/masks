...
module Masks
  ...
  class << self
    ...
    def request(request)
      ...
      session = model.new(config: configuration, request: request)
      session.mask!
      session
    end
    ...
    def mask(name, params: {}, **data)
      ...
      session =
        model.new(name: name, config: configuration, params: params, data: data)
      session.mask!
    ...
    def config_path=(value)
      @config_path =
        ...
        when Pathname
          ...
          raise Error::InvalidConfiguration.new(value)
        end
    end
    ...
    def load_config(dir)
      JSON.load(File.new(dir.join("masks.json"))).deep_symbolize_keys
    end
